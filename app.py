...
@app.route('/aforgotpassword',methods=['GET','POST'])
def aforgot():
    if request.method=='POST':
        username=request.form['username']
        cursor=mysql.connection.cursor()
        ...
        if (username,) in data:
            ...
            body=f'Reset the password using-{request.host+url_for("createpassword",token=token(username,120))}'
            sendmail(data,subject,body)
            flash(f'Reset link sent to your mail')
            return redirect(url_for('adminlogin'))
        else:
            return 'Invalid username'
    return render_template('forgot.html')
    ...
    try:
        ...
        if request.method=='POST':
            ...
            if npass==cpass:
                ...
                mysql.connection.commit()
                return 'Password reset Successfull go back to Login'
            else:
...
def conatact():
    if request.method=='POST':
        ...
        cursor=mysql.connection.cursor()
        nd=[name,email,subject,note]
        query='INSERT INTO contact (name, email, subject, note) VALUES (%s, %s, %s, %s)'
        cursor.execute(query,nd)
        mysql.connection.commit()
        cursor.close()
        flash(f'Details submitted successfully')
        return redirect(url_for('index'))
    return redirect(url_for('index'))


@app.route('/createpassword/<token>',methods=['GET','POST'])
def acreatepassword(token):
    try:
        s=Serializer(app.config['SECRET_KEY'])
        username=s.loads(token)['user']
        if request.method=='POST':
